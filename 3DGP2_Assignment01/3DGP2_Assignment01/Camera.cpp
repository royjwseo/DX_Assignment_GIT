...
void CThirdPersonCamera::UpdateByBullet(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	{
		XMFLOAT4X4 xmf4x4Rotate = Matrix4x4::Identity();
		XMFLOAT3 xmf3Right = pBulletShader->m_ppBullets[0]->GetRight();
		XMFLOAT3 xmf3Up = XMFLOAT3(0.f,1.f,0.f);
		XMFLOAT3 xmf3Look = pBulletShader->m_ppBullets[0]->GetLook();//(pTankPlayer->m_pPoshin->GetLook());
		...
		XMFLOAT3 xmf3Offset = Vector3::TransformCoord(m_xmf3Offset, xmf4x4Rotate);
		XMFLOAT3 xmf3Position = Vector3::Add(pBulletShader->m_ppBullets[0]->GetPosition(), xmf3Offset);
		XMFLOAT3 xmf3Direction = Vector3::Subtract(xmf3Position, m_xmf3Position, false);
		float fLength = Vector3::Length(xmf3Direction);
		xmf3Direction = Vector3::Normalize(xmf3Direction);
		float fTimeLagScale = (m_fTimeLag) ? fTimeElapsed * (1.0f / m_fTimeLag) : 1.0f;
...
void CThirdPersonCamera::SetLookAtByBullet(XMFLOAT3& xmf3LookAt)
{
	//CBulletsShader* pBulletShader = static_cast<CBulletsShader*>(m_pPlayer->m_pPlayerShader);
	//m_xmf3Position.y = m_xmf3Position.y + 0.5f;
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, m_pPlayer->GetUpVector());
	m_xmf3Right = XMFLOAT3(mtxLookAt._11, mtxLookAt._21, mtxLookAt._31);
	m_xmf3Up = XMFLOAT3(mtxLookAt._12, mtxLookAt._22, mtxLookAt._32);
	m_xmf3Look = XMFLOAT3(mtxLookAt._13, mtxLookAt._23, mtxLookAt._33);
...
FourthCamera::FourthCamera(CCamera* pCamera) : CCamera(pCamera)
{
	m_nMode = LEFT_CAMERA;
	if (pCamera)
	{
...
void FourthCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		XMFLOAT3 xmf3Right = m_pPlayer->GetRightVector();
...
void FourthCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	if (m_pPlayer)
	{
		XMFLOAT4X4 xmf4x4Rotate = Matrix4x4::Identity();
...
void FourthCamera::SetLookAt(XMFLOAT3& xmf3LookAt)
{
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, m_pPlayer->GetUpVector());
	m_xmf3Right = XMFLOAT3(mtxLookAt._11, mtxLookAt._21, mtxLookAt._31);
	m_xmf3Up = XMFLOAT3(mtxLookAt._12, mtxLookAt._22, mtxLookAt._32);
